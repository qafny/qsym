...
class StateCollector(ProgramVisitor):
    ...
    def __init__(self, kenv: dict, tenv: dict):
        ...
        self.tenv = tenv
        self.env = dict()
        # mapping from function names -> three items, states to different types
        # TyEn, TyNor, TyHad, TyAA
        # the first two items mapping from loci -> states
        # the third item is a list of predicates
        self.senv = [] # temp pre-env for a method
    ...
    def createRequireVars(self, l:list[QXCond]):
        ...
        for var in tmp:
            ...
            ty = TyEn(QXNum(1))
            sum = QXCon('k', QXCRange(QXNum(0), 
                        QXBin('^', QXNum(2), QXBind(v))))
            amp =  QXBin('/',
                        QXNum(1), 
                        QXUni('sqrt',  
                        QXBin(
            ...
            st = QXSum([sum], amp, [QXSKet([QXBind('k')])])
            result += [(locus, ty, st)]
        return result
    ...
    def findLocusTy(self, locus: list[QXQRange]):
    ...
    def findLocusSt(self, locus: list[QXQRange]):

        for elem, st in self.senv:
            vs = compareLocus(locus, elem)
            if vs == []:
                return st
        return None
    ...
    #    print('anything?', self.createRequireVars(ctx.conds()), self.createEnsureVars(ctx.conds()))
        self.senv += self.createRequireVars(ctx.conds())
        ...
        self.env.update({self.fvar:(self.senv,self.mkenv,self.pred)})
        return True
    ...
    def visitRequires(self, ctx: Programmer.QXRequires):
        ...
        if isinstance(ctx.spec(), QXQSpec):
            ...
            self.senv.append((ctx.spec().locus(), ctx.spec().qty(), ctx.spec().state()))
        ...
        if isinstance(ctx.spec(), QXComp):
            ...
            if isinstance(tyx, TyQ) and isinstance(tyy, TyQ):
                xS = self.findLocusSt([QXQRange(left, QXCRange(QXNum(0),tyx.flag()))])
                yS = self.findLocusSt([QXQRange(right, QXCRange(QXNum(0),tyy.flag()))])

                xT = self.findLocusTy([QXQRange(left, QXCRange(QXNum(0),tyx.flag()))])
                yT = self.findLocusTy([QXQRange(right, QXCRange(QXNum(0),tyy.flag()))])

                if xS is None and yS is None:
                    return False
                elif xS is None:
                    self.senv.append([QXQRange(left, QXCRange(QXNum(0),tyx.flag()))], yS)
                elif yS is None:
                    self.senv.append([QXQRange(left, QXCRange(QXNum(0), tyx.flag()))], xS)
                else:
                    # NEED TO CHECK the function compareType
    ...
    def visitEnsures(self, ctx: Programmer.QXEnsures):
        if isinstance(ctx.spec(), QXQSpec):
            ...
            self.mkenv.append((ctx.spec().locus(), ctx.spec().qty(), ctx.spec().state()))
