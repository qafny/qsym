...
method {:axiom} lambda(p : seq<bv1>) returns (p1: seq<bv1>)
  ensures |p1| == |p|
  ensures castBVInt(p1) == castBVInt(p) + 1

method conditionaltest5(n : nat, N: nat, q: seq<seq<seq<bv1>>>, p: seq<seq<seq<bv1>>>, amp: seq<seq<real>>, r: seq<bv1>, u: seq<bv1>)
        returns (q2: seq<seq<seq<seq<bv1>>>>, p2: seq<seq<seq<seq<bv1>>>>, u2: seq<seq<seq<seq<bv1>>>>, r2: seq<seq<seq<seq<bv1>>>>, amp2: seq<seq<seq<real>>>)
    requires |u| == |r| == 1
    requires castBVInt(u) == castBVInt(r) == 0
    requires |q| == |p| == |amp| == pow2(n)
    requires forall k :: 0 <= k < pow2(n) ==> |q[k]| == |p[k]| == |amp[k]| == pow2(n)
    requires forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> |q[k][j]| == |p[k][j]| == n
    ...
    ensures |p2| == |q2| == |r2| == |u2| == |amp2| == pow2(n)
    ensures forall k :: 0 <= k < pow2(n) ==> |p2[k]| == |q2[k]| == |r2[k]| == |u2[k]| == |amp2[k]| == pow2(n)
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> |p2[k][j]| == |q2[k][j]| == |u2[k][j]| == |r2[k][j]| == |amp2[k][j]| == pow2(1)
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> |p2[k][j][l]| ==  |q2[k][j][l]| == n
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> |r2[k][j][l]| ==  |u2[k][j][l]| == 1
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> castBVInt(q2[k][j][l]) == k
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> castBVInt(p2[k][j][l]) == if castBVInt(q2[k][j][l]) < N && r2[k][j][l][0] == 1 then j + 1 else j
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> castBVInt(u2[k][j][l]) == if castBVInt(q2[k][j][l]) < N then 1 else 0
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> castBVInt(r2[k][j][l]) == if castBVInt(q2[k][j][l]) < N then l else 0
    ensures forall k :: 0 <= k < pow2(n) ==> forall j :: 0 <= j < pow2(n) ==> forall l :: 0 <= l < pow2(1) ==> 
                  amp2[k][j][l] == if castBVInt(q2[k][j][l]) < N then 1.0/(pow2(n) as real) * 1.0/(sqrt(pow2(|r2[k][j][l]|) as real)) else 1.0/(pow2(n) as real)

  {
    ...
    while (tmp < |p|)
        ...
        invariant forall k :: 0 <= k < |p4| ==> forall j :: 0 <= j < |p4[k]| ==> forall l :: 0 <= l < |p4[k][j]| ==> castBVInt(p4[k][j][l]) == if castBVInt(q4[k][j][l]) < N && r4[k][j][l][0] == 1 then j + 1 else j
        invariant forall k :: 0 <= k < |p4| ==> forall j :: 0 <= j < |p4[k]| ==> forall l :: 0 <= l < |p4[k][j]| ==> castBVInt(q4[k][j][l]) == k
        invariant forall k :: 0 <= k < |p4| ==> forall j :: 0 <= j < |p4[k]| ==> forall l :: 0 <= l < |p4[k][j]| ==> castBVInt(u4[k][j][l]) == if castBVInt(q[k][j]) < N then 1 else 0
        invariant forall k :: 0 <= k < |p4| ==> forall j :: 0 <= j < |p4[k]| ==> forall l :: 0 <= l < |p4[k][j]| ==> castBVInt(r4[k][j][l]) == if castBVInt(q[k][j]) < N then l else 0
        invariant forall k :: 0 <= k < |p4| ==> forall j :: 0 <= j < |p4[k]| ==> forall l :: 0 <= l < |p4[k][j]| ==> amp4[k][j][l] == if castBVInt(q[k][j]) < N then 1.0/(pow2(n) as real) * 1.0/(sqrt(pow2(|r4[k][j][l]|) as real)) else 1.0/(pow2(n) as real)
    {
        var tmpu := [];
        var tmpr := [];
        var tmpp := [];
        ...
        while (tmp1 < |p[tmp]|)
            ...
            invariant forall k :: 0 <= k < tmp1 ==> forall j :: 0 <= j < pow2(1) ==> castBVInt(tmpp[k][j]) == if castBVInt(tmpq[k][j]) < N  && tmpr[k][j][0] == 1 then k + 1 else k 
            invariant forall k :: 0 <= k < tmp1 ==> forall j :: 0 <= j < pow2(1) ==> castBVInt(tmpr[k][j]) == if castBVInt(q[tmp][k]) < N then j else 0
            invariant forall k :: 0 <= k < tmp1 ==> forall j :: 0 <= j < pow2(1) ==> castBVInt(tmpu[k][j]) == if castBVInt(q[tmp][k]) < N then 1 else 0
            invariant forall k :: 0 <= k < tmp1 ==> forall j :: 0 <= j < pow2(1) ==> tmpamp[k][j] == if castBVInt(q[tmp][k]) < N then  1.0/(pow2(n) as real) * 1.0/(sqrt(pow2(|tmpu[k][j]|) as real)) else 1.0/(pow2(n) as real)
        {
            var res := bool2BV1(castBVInt(q[tmp][tmp1]) < N);
            var tmmmu := duplicateSeq(res, pow2(|r|));
            ...
            if (castBVInt(q[tmp][tmp1]) < N){
                tmmmr := partialcastEn1toEn2(r3[tmp][tmp1]);
                tmmpamp := ampMul(tmmpamp, pow2(|r3[tmp][tmp1]|), r3[tmp][tmp1]);
                var tmploop := 0;
                var tmmmpp := [];
                while (tmploop < |tmmmr|)
                  invariant 0 <= tmploop <= |tmmmr|
                  invariant |tmmmpp| == tmploop
                  invariant forall k :: 0 <= k < |tmmmpp| ==> |tmmmpp[k]| == |tmmmp[k]|
                  invariant forall k :: 0 <= k < |tmmmpp| ==> castBVInt(tmmmpp[k]) == if tmmmr[k][0] == 1 then castBVInt(tmmmp[k]) + 1 else castBVInt(tmmmp[k])
                {
                    if (tmmmr[tmploop][0] == 1){
                       var lambdares := lambda(tmmmp[tmploop]);
                       tmmmpp := tmmmpp + [lambdares];
                    }
                    else{
                      tmmmpp:= tmmmpp + [tmmmp[tmploop]];
                    }
                    tmploop := tmploop + 1;
                }
                tmmmp := tmmmpp;
